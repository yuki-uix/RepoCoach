...
真实仓库（非 fixture）默认改由模型生成候选：把目录树摘要、package 摘要与 barrel 穿透后的真实定义文件/符号一并交给模型（仓库数据经 `REPO_DATA` 标记包裹，system prompt 固定不变），模型只允许在给定文件列表内挑选 `entryFiles`。模型输出仍过同一道出口闸——zod 校验、文件存在性过滤、id 与标题去重；非法或空输出重试一次后回落上述确定性启发式。fixture 仓库走预置候选白名单，不调用模型；`list`/`show` 等不生成候选的命令也不会要求 API key。同名标题的候选会附带其定义文件路径以消歧，保证列表内不出现两条完全相同的标题。
